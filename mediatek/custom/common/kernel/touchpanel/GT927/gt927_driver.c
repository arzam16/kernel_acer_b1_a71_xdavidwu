...
//xdavidwu tpd history
static s32 lastest_x = 0;
static s32 lastest_y = 0;
//xdavidwu
...
#if GTP_ESD_PROTECT
#define TPD_ESD_CHECK_CIRCLE        10
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    */
...
#if GTP_DIFFDATA_PROC
    gt927_diffdata_proc = create_proc_entry(GT927_DIFFDATA_PROC_FILE, 0444, NULL);
...
#endif
#if GTP_RAWDATA_PROC
    gt927_rawdata_proc = create_proc_entry(GT927_RAWDATA_PROC_FILE, 0444, NULL);
    ...
    gt927_PanelType_proc = create_proc_entry(GT927_PANELTYPE_PROC_FILE, 0666, NULL);
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    charger_state = upmu_get_PCHR_CHRDET();
...
printk("[SWEEP2WAKE]: tpd down\n");
		...
		if (sweep2wake) {
			...
			detect_sweep2wake(y, x, jiffies, id);
#else
...
//xdavidwu
#ifdef CONFIG_XDW_TS_FIX_4_2
                tpd_down((int)input_y*1024/600,(int)input_x*600/1024, input_w, id);
#else
                tpd_down(input_x,input_y, input_w, id);
#endif
            }
#ifdef CONFIG_XDW_TS_FIX_4_2 //for lastest touch record
                lastest_x=(int)input_y*1024/600;
		lastest_y=(int)input_x*600/1024;
#else
                lastest_x=input_x;
		lastest_y=input_y;
#endif
